...
const QueueList = ({ queues, onSelect }) => {
  if (!queues || !Array.isArray(queues) || queues.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-3xl border border-dashed border-slate-300">
        ...
        <h3 className="text-lg font-semibold text-slate-700">No queues found</h3>
        <p className="text-slate-500">Create your first queue to start managing customers</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {queues.map((queue) => (
        <Card key={queue._id} className="p-6 hover:shadow-lg transition-shadow border-slate-100 group">
