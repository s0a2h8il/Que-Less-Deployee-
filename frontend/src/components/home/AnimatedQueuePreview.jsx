...
import { Clock, Users, Zap, ShieldCheck, ArrowRight, TrendingUp, History, PlusCircle, BarChart3, Building2 } from "lucide-react";
...
const AdminQuickActionsCard = ({ stats, role }) => {
    ...
    >
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="p-6 rounded-[2rem] bg-white border border-black/5 shadow-sm hover:shadow-md transition-shadow">
          <p className="text-[10px] font-black uppercase tracking-widest opacity-40 mb-3">{isSuperAdmin ? "Total Users" : "Active Queues"}</p>
          <div className="flex items-center gap-3">
            <span className="text-4xl font-black" style={{ color: palette.ink }}>{isSuperAdmin ? (stats.totalPeople || 0) : stats.activeQueues}</span>
            <div className="h-8 w-[1px] bg-slate-200" />
            <span className="text-xs font-bold text-[var(--mint-soft)]">{isSuperAdmin ? "System" : "Live"}</span>
          </div>
        </div>
        <div className="p-6 rounded-[2rem] bg-white border border-black/5 shadow-sm hover:shadow-md transition-shadow">
          <p className="text-[10px] font-black uppercase tracking-widest opacity-40 mb-3">{isSuperAdmin ? "Verified Biz" : "Total People"}</p>
          <div className="flex items-center gap-3">
            <span className="text-4xl font-black" style={{ color: palette.ink }}>{isSuperAdmin ? (stats.verifiedBusinesses || 0) : stats.totalPeople}</span>
            <div className="h-8 w-[1px] bg-slate-200" />
            {isSuperAdmin ? <ShieldCheck size={16} className="text-[var(--mint-soft)]" /> : <Users size={16} className="text-slate-300" />}
          </div>
        </div>
      ...
      <div className="relative z-10 flex flex-col md:flex-row gap-6">
        <Link to={isSuperAdmin ? "/admin/superadmin" : "/admin/dashboard"} className="flex-[1.5] group">
          <div className="h-full flex flex-col justify-between p-7 rounded-[2rem] bg-gradient-to-br from-[var(--night-ink)] to-[var(--slate-core)] text-white transition-all shadow-2xl group-hover:shadow-[var(--night-ink)]/30 group-hover:-translate-y-1 border border-white/10 relative overflow-hidden">
            ...
            <div>
              <span className="text-2xl font-black tracking-tighter block mb-1">{isSuperAdmin ? "Platform Panel" : "Launch Dashboard"}</span>
              <p className="text-xs font-bold opacity-60">{isSuperAdmin ? "Global system governance & control" : "Full business analytics & queue control"}</p>
            </div>
          </div>
        </Link>
        <div className="flex-1 grid grid-cols-1 gap-4">
          <Link to={isSuperAdmin ? "/admin/superadmin?tab=businesses" : "/admin/dashboard?tab=create-queue"} className="group h-full">
            <div className="h-full flex items-center gap-4 p-5 rounded-2xl bg-white border border-black/5 hover:border-[var(--mint-soft)]/30 transition-all shadow-sm hover:shadow-md">
              <div className="p-3 rounded-xl bg-[var(--mint-soft)]/10 text-[var(--mint-soft)]">
                {isSuperAdmin ? <Building2 size={24} /> : <PlusCircle size={24} />}
              </div>
              <span className="text-sm font-black tracking-tight" style={{ color: palette.ink }}>{isSuperAdmin ? "Verify Biz" : "Create Queue"}</span>
            </div>
          </Link>
          <Link to={isSuperAdmin ? "/admin/superadmin?tab=users" : "/analytics"} className="group h-full">
            <div className="h-full flex items-center gap-4 p-5 rounded-2xl bg-white border border-black/5 hover:border-[var(--signal-blue)]/30 transition-all shadow-sm hover:shadow-md">
              <div className="p-3 rounded-xl bg-[var(--signal-blue)]/10 text-[var(--signal-blue)]">
                {isSuperAdmin ? <Users size={24} /> : <BarChart3 size={24} />}
              </div>
              <span className="text-sm font-black tracking-tight" style={{ color: palette.ink }}>{isSuperAdmin ? "Manage Users" : "Queue Reports"}</span>
            </div>
          </Link>
        </div>
      </div>
...
const AnimatedQueuePreview = () => {
  ...
  if (!hasRealData) {
    return (
      <div className="relative mx-auto" style={{ maxWidth: 580 }}>
        {isAdmin ? <AdminQuickActionsCard stats={adminStats} role={user?.role} /> : <EmptyQueueCard title="No live queue data" message="Join a queue to see your position, wait time, and nearby customers here." />}
      </div>
    );
  }
