...
const BusinessCard = ({ business, index }) => {
  const { name, category, city, address, rating, isVerified, isActive } = business;

  const queueStatus = isActive ? "active" : "closed";
  ...
  return (
    ...
    >
      <div
        ...
        onMouseEnter={e => { e.currentTarget.style.boxShadow = "var(--shadow-lg)"; e.currentTarget.style.borderColor = "rgba(58,160,255,0.20)"; }}
        onMouseLeave={e => { e.currentTarget.style.boxShadow = "var(--shadow-sm)"; e.currentTarget.style.borderColor = "var(--border)"; }}
      >
        {/* ── Card Header ─────────────────────────────────────── */}
        <div
          className={cn("relative h-28 overflow-hidden bg-gradient-to-br", getGradient(category))}
        ...
        >
          ...
          {/* Category avatar */}
          <div
            className="absolute -bottom-5 left-5 h-11 w-11 rounded-xl flex items-center justify-center font-bold text-lg shadow-md"
            style={{
              background: "var(--surface)",
              border: "1px solid var(--border)",
              color: "#3AA0FF",
        ...
        <div className="p-5 pt-9 flex-1 flex flex-col">
          ...
          <div className="flex items-center gap-1.5 mb-4" style={{ color: "var(--text-muted)" }}>
            ...
            <span className="text-[13px] truncate">{address}, {city}</span>
          </div>
          ...
          <div className="mt-auto">
            {isActive ? (
              <Link
                to={`/explore?search=${encodeURIComponent(name)}`}
              ...
              >
                ...
                />
                <div className="relative flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold" style={{ color: "#0B1320", fontFamily: "var(--font-heading)" }}>
