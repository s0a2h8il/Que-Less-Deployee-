...
import { BellRing, Clock, Users, LayoutDashboard, BarChart3, PlusCircle, Zap, ShieldCheck, ArrowRight, TrendingUp, History } from "lucide-react";
  ...
  >
    ...
    <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
      ...
      <div className="p-6 rounded-[2rem] bg-[var(--night-ink)] text-white shadow-xl">
        ...
        <div className="flex items-center justify-between mb-2">
          <span className="text-xl font-black tracking-tight">+14.2%</span>
          <TrendingUp size={18} className="text-[var(--mint-soft)]" />
        </div>
        <div className="h-1 w-full bg-white/10 rounded-full overflow-hidden">
          <motion.div initial={{ width: 0 }} animate={{ width: "65%" }} transition={{ duration: 1.5 }} className="h-full bg-[var(--mint-soft)]" />
        </div>
      </div>
    </div>
    ...
    <div className="relative z-10 flex flex-col md:flex-row gap-6">
      ...
      <div className="flex-1 grid grid-cols-1 gap-4">
        <Link to="/admin/dashboard" className="group h-full">
...
const AnimatedQueuePreview = () => {
  ...
  const [adminStats, setAdminStats] = useState({ activeQueues: 0, totalPeople: 0 });
  ...
  return (
    <div className="relative mx-auto" style={{ maxWidth: 580 }}>
      ...
      <motion.div animate={{ y: [0, -12, 0] }} transition={{ duration: 4, repeat: Infinity }} className="absolute z-20 flex h-16 w-16 flex-col items-center justify-center rounded-full text-white shadow-xl" style={{ bottom: "-1.5rem", left: "-1.5rem", background: "linear-gradient(135deg, #E07A5F 0%, #F2CC8F 100%)", fontFamily: "var(--font-heading)" }}>
        <div style={{ fontSize: "1.3rem", fontWeight: 900 }}>{aheadCount + 1}</div>
        <div style={{ fontSize: "8px", fontWeight: 900, textTransform: "uppercase" }}>Pos</div>
      </motion.div>

      <motion.div animate={{ y: [0, 10, 0] }} transition={{ duration: 5, repeat: Infinity }} className="absolute z-20 flex h-24 w-24 items-center justify-center rounded-full text-white shadow-2xl" style={{ top: "-1.5rem", right: "-1.5rem", background: "linear-gradient(135deg, #3D405B 0%, #81B29A 100%)" }}>
        <div className="text-center">
          <div className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-1">Serving</div>
          <div style={{ fontFamily: "var(--font-heading)", fontSize: "2rem", fontWeight: 900 }}>#{currentToken}</div>
        </div>
      </motion.div>

      <div style={{ background: "rgba(255,255,255,0.94)", border: "1px solid rgba(61,64,91,0.18)", borderRadius: "2.5rem", boxShadow: "0 30px 80px rgba(61,64,91,0.16)", backdropFilter: "blur(24px)", overflow: "hidden" }}>
        <div className="flex flex-col md:flex-row">
          <div className="flex-1 flex flex-col justify-center items-center text-center" style={{ background: "linear-gradient(135deg, rgba(129,178,154,0.12) 0%, rgba(190,227,248,0.20) 100%)", padding: "2.5rem" }}>
            <div className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.15em] mb-8 bg-white/80 border border-black/5 shadow-sm">
              <span className="w-1.5 h-1.5 rounded-full animate-pulse bg-[var(--mint-soft)]" />
              Live Tracking
            </div>
            <h4 className="text-xs font-black uppercase tracking-[0.2em] opacity-40 mb-3" style={{ color: palette.ink, fontFamily: "var(--font-heading)" }}>Assigned Token</h4>
            <div style={{ fontFamily: "var(--font-heading)", fontSize: "4.5rem", fontWeight: 900, color: palette.ink, letterSpacing: "-0.05em", lineHeight: 0.9 }}>
              #{members.find(m => m.label === "You")?.token || "-"}
            </div>
          </div>

          <div className="flex-1 p-8 md:border-l border-black/5 flex flex-col justify-center">
            {[
        ...
        <div className="p-8 border-t border-black/5 bg-white/40">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {members.map((m, i) => (
              <motion.div key={m.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.1 }} className="flex items-center justify-center p-4 rounded-2xl border bg-white shadow-sm">
                <div className="flex items-center gap-3">
